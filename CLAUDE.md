...
## No hard-wrapping in Markdown, PR/issue bodies, or comments

Don't insert manual line breaks inside a paragraph to keep lines under some fixed column width. Write each paragraph as one logical line (or wrap only at natural markdown boundaries — list items, headings, code blocks) and let the renderer (GitHub, an editor, a terminal) soft-wrap it for display. This applies to `.md` files in the repo (including `CLAUDE.md` and `README.md`), and to PR bodies, issue bodies, and comments opened via `gh`. Hard-wrapped prose looks fine in a narrow terminal but reflows badly and reads as broken formatting anywhere it's rendered wider (GitHub's web UI, most editors).
